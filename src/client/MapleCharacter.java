...
public class MapleCharacter extends AnimatedMapleMapObject implements Serializable, MapleCharacterLook {
    ...
    public static MapleCharacter getDefault(final MapleClient client, final JobType type) {
        ...
        ret.job = (short) (type == JobType.林芝林 ? MapleJob.林芝林4转.getId() : type.id);
        ret.meso = 100000;
        ret.level = 1;
        ret.remainingAp = 0;
        ret.fame = 0;
    ...
    public static void saveNewCharToDB(final MapleCharacter chr, final JobType type, short db, int keymapType) {
        ...
        try {
            ...
            pse = con.prepareStatement("INSERT INTO queststatusmobs(`queststatusmobid`, `queststatusid`, `mob`, `count`) VALUES (DEFAULT, ?, ?, ?)");
            ps.setInt(1, chr.id);
    ...
    public final void QuestInfoPacket(final tools.data.MaplePacketLittleEndianWriter mplew) {
        if (MapleJob.is林芝林(getJob())) {
            int beast = MapleJob.is林芝林(getJob()) ? 1 : 0;
...
//            client.getSession().write(EffectPacket.updateDeathCount(99)); //for fun
    ...
    public void changeJob(short newJob) {
        try {
            ...
            if (MapleJob.is影武者(newJob)) {
                subcategory = 1;
            } else if (MapleJob.飞侠.getId() != newJob) {
                subcategory = 0;
            }
            ...
            if (!MapleJob.isBeginner(newJob) && newJob != MapleJob.血刀.getId() && !MapleJob.is神之子(newJob) && !MapleJob.is林芝林(newJob)) {
                addJobSkills();
                ...
                //自定義補滿技能點
                if (newJob == MapleJob.见习刀客.getId() || newJob == MapleJob.双刀客.getId() || newJob == MapleJob.双刀侠.getId()) {
                    remainingSp[GameConstants.getSkillBookByJob(newJob)] += 5;
                    if (newJob == MapleJob.双刀客.getId()) {
            ...
            switch (job) {
                ...
                default:
                    System.err.println("职业 " + MapleJob.getById(job).name() + " 未處理轉職HPMP增加");
            }
            if (maxhp >= 500000) {
                maxhp = 500000;
            }
    ...
    public void gainExpMonster(int gain, final boolean show, final boolean white, byte pty, final byte Class_Bonus_EXP, final byte Premium_Bonus_EXP_PERCENT, MapleMonster mob) {
        ...
        if (skilLevel > 0 && MapleJob.is双弩精灵(getJob())) {
...
//            updateDonorMedal(getInventory(MapleInventoryType.EQUIPPED).findById(1142229));
    ...
    public final void expirationTask(boolean pending, boolean firstLoad) {
        ...
        for (final MapleInventoryType inv : MapleInventoryType.values()) {
            for (final Item item : getInventory(inv)) {
                ...
                if ((expiration != -1 && !ItemConstants.类型.宠物(item.getItemId()) && currenttime > expiration) || (firstLoad && ii.isLogoutExpire(item.getItemId()))) {
    ...
    public void levelUp() {
        ...
        if (job == 3001 || job == 10000) {// 恶魔猎手,神之子 的新手
            maxhp += Randomizer.rand(52, 56);
        } else if (MapleJob.isBeginner(job)) { // 新手 (無惡魔,神之子)
            maxhp += Randomizer.rand(12, 16);
            maxmp += Randomizer.rand(10, 12);
        } else if (MapleJob.is劍士(job) && (MapleJob.is冒險家(job) || MapleJob.is皇家騎士團(job))) { // 战士、聖魂劍士
            maxhp += Randomizer.rand(48, 52);
            maxmp += Randomizer.rand(4, 6);
        } else if (MapleJob.is狂狼勇士(job)) { // 战神
            maxhp += Randomizer.rand(50, 52);
            maxmp += Randomizer.rand(4, 6);
        } else if (MapleJob.is惡魔殺手(job)) { // 恶魔猎手
            maxhp += Randomizer.rand(48, 52);
        } else if (MapleJob.is惡魔復仇者(job)) { // 惡魔復仇者
            maxhp += Randomizer.rand(70, 105);
        } else if (MapleJob.is米哈逸(job)) { // 米哈尔
            maxhp += Randomizer.rand(48, 52);
            maxmp += Randomizer.rand(4, 6);
            ...
            maxhp += Randomizer.rand(48, 52);
            maxmp += Randomizer.rand(4, 6);
        } else if (MapleJob.is法師(job) && (MapleJob.is冒險家(job) || MapleJob.is皇家騎士團(job))) { // 魔法师、烈焰巫師
            maxhp += Randomizer.rand(10, 14);
            maxmp += Randomizer.rand(48, 52);
        } else if (MapleJob.is龍魔導士(job)) { // 龙神
            maxhp += Randomizer.rand(12, 16);
            maxmp += Randomizer.rand(50, 52);
        } else if (MapleJob.is夜光(job)) { // 夜光法师
            maxhp += Randomizer.rand(25, 40);
            maxmp += Randomizer.rand(60, 100);
        } else if (MapleJob.is煉獄巫師(job)) { // 煉獄巫師
            maxhp += Randomizer.rand(20, 24);
            maxmp += Randomizer.rand(42, 44);
        } else if (MapleJob.is陰陽師(job) || MapleJob.is凱內西斯(job)) { // 阴阳师、超能力者
            maxhp += Randomizer.rand(48, 52);
        ...
        } else if (MapleJob.is拳霸(job)) { // 冲锋队长
        ...
        } else if (MapleJob.is隱月(job)) { // 隐月
            maxhp += Randomizer.rand(66, 77);
            maxmp += Randomizer.rand(44, 57);
        } else if (MapleJob.is尖兵(job)) { // 尖兵
            maxhp += Randomizer.rand(100, 130);
            maxmp += Randomizer.rand(10, 15);
        } else if (MapleJob.is天使破壞者(job)) { // 林芝林
            maxhp += Randomizer.rand(56, 67);
        } else if (MapleJob.is林芝林(job)) { // 林芝林
            maxhp += Randomizer.rand(37, 41);
            maxmp += Randomizer.rand(20, 24);
        } else {
            System.err.println("职业 " + MapleJob.getById(job).name() + " 未處理升級HPMP增加");
        }
    ...
    public void autoJob() {
        if (MapleJob.is英雄(job)) {
            switch (getLevel()) {
                case 60:
                    changeJob((short) MapleJob.勇士.getId());
                    break;
                case 100:
            ...
            switch (getLevel()) {
                case 60:
                    ...
                    break;
                case 100:
                    changeJob((short) MapleJob.圣骑士.getId());
                    break;
            }
        } else if (MapleJob.is黑騎士(job)) {
            switch (getLevel()) {
                case 60:
                    changeJob((short) MapleJob.龙骑士.getId());
                    break;
                case 100:
            ...
            switch (getLevel()) {
                case 60:
                    changeJob((short) MapleJob.火毒巫师.getId());
                    break;
                case 100:
                    changeJob((short) MapleJob.火毒魔导士.getId());
                    break;
            }
        } else if (MapleJob.is大魔導士_冰雷(job)) {
            switch (getLevel()) {
                case 60:
                    changeJob((short) MapleJob.冰雷巫师.getId());
                    break;
                case 100:
                    changeJob((short) MapleJob.冰雷魔导士.getId());
                    break;
            }
            ...
            switch (getLevel()) {
                case 60:
                    changeJob((short) MapleJob.射手.getId());
                    break;
                case 100:
            ...
            switch (getLevel()) {
                case 60:
                    changeJob((short) MapleJob.游侠.getId());
                    break;
                case 100:
            ...
            switch (getLevel()) {
                case 60:
                    changeJob((short) MapleJob.无影人.getId());
                    break;
                case 100:
                    changeJob((short) MapleJob.隐士.getId());
                    break;
            }
        } else if (MapleJob.is暗影神偷(job)) {
            switch (getLevel()) {
                case 60:
                    changeJob((short) MapleJob.独行客.getId());
                    break;
                case 100:
                    changeJob((short) MapleJob.侠盗.getId());
                    break;
            }
        } else if (MapleJob.is影武者(job) || (this.subcategory == 1 && job == 400)) {
            switch (getLevel()) {
                case 20:
                    changeJob((short) MapleJob.见习刀客.getId());
                    break;
                case 30:
                    changeJob((short) MapleJob.双刀客.getId());
                    break;
                case 45:
                    changeJob((short) MapleJob.双刀侠.getId());
                    break;
                case 60:
                    changeJob((short) MapleJob.血刀.getId());
                    break;
                case 100:
                    changeJob((short) MapleJob.暗影双刀.getId());
                    break;
            }
        } else if (MapleJob.is拳霸(job)) {
            switch (getLevel()) {
                case 60:
                    changeJob((short) MapleJob.斗士.getId());
                    break;
                case 100:
                    changeJob((short) MapleJob.冲锋队长.getId());
                    break;
            }
        } else if (MapleJob.is槍神(job)) {
            switch (getLevel()) {
                case 60:
                    changeJob((short) MapleJob.大副.getId());
                    break;
                case 100:
                    changeJob((short) MapleJob.船长.getId());
                    break;
            }
        } else if (MapleJob.is重砲指揮官(job)) {
            switch (getLevel()) {
                case 30:
                    changeJob((short) MapleJob.火炮手2转.getId());
                    break;
                case 60:
                    changeJob((short) MapleJob.毁灭炮手3转.getId());
                    break;
                case 100:
                    changeJob((short) MapleJob.神炮王.getId());
                    break;
            }
        } else if (MapleJob.is蒼龍俠客(job)) {
            switch (getLevel()) {
                case 30:
                    changeJob((short) MapleJob.龙的传人2转.getId());
                    break;
                case 60:
                    changeJob((short) MapleJob.龙的传人3转.getId());
                    break;
                case 100:
                    changeJob((short) MapleJob.龙的传人4转.getId());
                    break;
            }
        } else if (MapleJob.is聖魂劍士(job)) {
            switch (getLevel()) {
                case 30:
                    changeJob((short) MapleJob.魂骑士2转.getId());
                    break;
                case 60:
                    changeJob((short) MapleJob.魂骑士3转.getId());
                    break;
                case 100:
                    changeJob((short) MapleJob.魂骑士4转.getId());
                    break;
            }
        } else if (MapleJob.is烈焰巫師(job)) {
            switch (getLevel()) {
                case 30:
                    changeJob((short) MapleJob.炎术士2转.getId());
                    break;
                case 60:
                    changeJob((short) MapleJob.炎术士3转.getId());
                    break;
                case 100:
                    changeJob((short) MapleJob.炎术士4转.getId());
                    break;
            }
        } else if (MapleJob.is破風使者(job)) {
            switch (getLevel()) {
                case 30:
                    changeJob((short) MapleJob.风灵使者2转.getId());
                    break;
                case 60:
                    changeJob((short) MapleJob.风灵使者3转.getId());
                    break;
                case 100:
                    changeJob((short) MapleJob.风灵使者4转.getId());
                    break;
            }
        } else if (MapleJob.is暗夜行者(job)) {
            switch (getLevel()) {
                case 30:
                    changeJob((short) MapleJob.夜行者2转.getId());
                    break;
                case 60:
                    changeJob((short) MapleJob.夜行者3转.getId());
                    break;
                case 100:
                    changeJob((short) MapleJob.夜行者4转.getId());
                    break;
            }
        } else if (MapleJob.is閃雷悍將(job)) {
            switch (getLevel()) {
                case 30:
                    changeJob((short) MapleJob.奇袭者2转.getId());
                    break;
                case 60:
                    changeJob((short) MapleJob.奇袭者3转.getId());
                    break;
                case 100:
                    changeJob((short) MapleJob.奇袭者4转.getId());
                    break;
            }
        } else if (MapleJob.is狂狼勇士(job)) {
            switch (getLevel()) {
                case 10:
                    changeJob((short) MapleJob.战神1转.getId());
                    break;
                case 30:
                    changeJob((short) MapleJob.战神2转.getId());
                    break;
                case 60:
                    changeJob((short) MapleJob.战神3转.getId());
                    break;
                case 100:
                    changeJob((short) MapleJob.战神4转.getId());
                    break;
            }
        } else if (MapleJob.is龍魔導士(job)) {
            switch (getLevel()) {
                case 10:
                    changeJob((short) MapleJob.龙神1转.getId());
                    break;
                case 20:
            ...
            switch (getLevel()) {
                case 10:
                    changeJob((short) MapleJob.双弩精灵1转.getId());
                    break;
                case 30:
                    changeJob((short) MapleJob.双弩精灵2转.getId());
                    break;
                case 60:
                    changeJob((short) MapleJob.双弩精灵3转.getId());
                    break;
                case 100:
                    changeJob((short) MapleJob.双弩精灵4转.getId());
                    break;
            }
        } else if (MapleJob.is幻影俠盜(job)) {
            switch (getLevel()) {
            ...
            switch (getLevel()) {
                case 30:
                    changeJob((short) MapleJob.恶魔猎手2转.getId());
                    break;
                case 60:
                    changeJob((short) MapleJob.恶魔猎手3转.getId());
                    break;
                case 100:
            ...
            switch (getLevel()) {
                case 30:
                    changeJob((short) MapleJob.唤灵法师2转.getId());
                    break;
                case 60:
                    changeJob((short) MapleJob.唤灵法师3转.getId());
                    break;
                case 100:
                    changeJob((short) MapleJob.唤灵法师4转.getId());
                    break;
            }
        } else if (MapleJob.is狂豹獵人(job)) {
            switch (getLevel()) {
                case 30:
                    changeJob((short) MapleJob.豹弩游侠2转.getId());
                    break;
                case 60:
                    changeJob((short) MapleJob.豹弩游侠3转.getId());
                    break;
                case 100:
                    changeJob((short) MapleJob.豹弩游侠4转.getId());
                    break;
            }
        } else if (MapleJob.is機甲戰神(job)) {
            switch (getLevel()) {
                case 30:
                    changeJob((short) MapleJob.机械师2转.getId());
                    break;
                case 60:
                    changeJob((short) MapleJob.机械师3转.getId());
                    break;
                case 100:
                    changeJob((short) MapleJob.机械师4转.getId());
                    break;
            }
        } else if (MapleJob.is尖兵(job)) {
            switch (getLevel()) {
                case 10:
                    changeJob((short) MapleJob.尖兵1转.getId());
                    break;
                case 30:
                    changeJob((short) MapleJob.尖兵2转.getId());
                    break;
                case 60:
                    changeJob((short) MapleJob.尖兵3转.getId());
                    break;
                case 100:
                    changeJob((short) MapleJob.尖兵4转.getId());
                    break;
            }
        } else if (MapleJob.is劍豪(job)) {
            switch (getLevel()) {
            ...
            switch (getLevel()) {
                case 10:
                    changeJob((short) MapleJob.狂龙战士1转.getId());
                    break;
                case 30:
                    changeJob((short) MapleJob.狂龙战士2转.getId());
                    break;
                case 60:
                    changeJob((short) MapleJob.狂龙战士3转.getId());
                    break;
                case 100:
                    changeJob((short) MapleJob.狂龙战士4转.getId());
                    break;
            }
        } else if (MapleJob.is天使破壞者(job)) {
            switch (getLevel()) {
                case 10:
                    changeJob((short) MapleJob.爆莉萌天使1转.getId());
                    break;
                case 30:
                    changeJob((short) MapleJob.爆莉萌天使2转.getId());
                    break;
                case 60:
                    changeJob((short) MapleJob.爆莉萌天使3转.getId());
                    break;
                case 100:
                    changeJob((short) MapleJob.爆莉萌天使4转.getId());
                    break;
            }
        } else if (MapleJob.is林芝林(job)) {
            switch (getLevel()) {
                case 10:
                    changeJob((short) MapleJob.林芝林1转.getId());
                    break;
                case 30:
                    changeJob((short) MapleJob.林芝林2转.getId());
                    break;
                case 60:
                    changeJob((short) MapleJob.林芝林3转.getId());
                    break;
                case 100:
                    changeJob((short) MapleJob.林芝林4转.getId());
                    break;
            }
        } else if (MapleJob.is凱內西斯(job)) {
            switch (getLevel()) {
                case 10:
                    changeJob((short) MapleJob.超能力者1转.getId());
                    break;
                case 30:
                    changeJob((short) MapleJob.超能力者2转.getId());
                    break;
                case 60:
                    changeJob((short) MapleJob.超能力者3转.getId());
                    break;
                case 100:
                    changeJob((short) MapleJob.超能力者4转.getId());
                    break;
            }
        }
    }
    ...
    public Triple<List<MapleRing>, List<MapleRing>, List<MapleRing>> getRings(boolean equip) {
        ...
        for (Item ite : equipped) {
            ...
            if (item.getRing() != null) {
                ...
                if (ItemConstants.类型.特效戒指(item.getItemId())) {
                    if (equip) {
                        if (ItemConstants.类型.戀人戒指(item.getItemId())) {
                            crings.add(ring);
                        } else if (ItemConstants.类型.友谊戒指(item.getItemId())) {
                            frings.add(ring);
                        } else if (ItemConstants.类型.結婚戒指(item.getItemId())) {
                            mrings.add(ring);
                        }
                    } else {
                        if (crings.isEmpty() && ItemConstants.类型.戀人戒指(item.getItemId())) {
                            crings.add(ring);
                        } else if (frings.isEmpty() && ItemConstants.类型.友谊戒指(item.getItemId())) {
                            frings.add(ring);
                        } else if (mrings.isEmpty() && ItemConstants.类型.結婚戒指(item.getItemId())) {
                            mrings.add(ring);
        ...
        if (equip) {
            ...
            for (Item ite : iv.list()) {
                Equip item = (Equip) ite;
                if (item.getRing() != null && ItemConstants.类型.戀人戒指(item.getItemId())) {
                    ring = item.getRing();
                    ring.setEquipped(false);
                    if (ItemConstants.类型.友谊戒指(item.getItemId())) {
                        frings.add(ring);
                    } else if (ItemConstants.类型.戀人戒指(item.getItemId())) {
                        crings.add(ring);
                    } else if (ItemConstants.类型.結婚戒指(item.getItemId())) {
                        mrings.add(ring);
                    }
    ...
    public Map<Byte, Integer> getSecondEquips(boolean fusionAnvil) {
        ...
        for (final Item item : inventory[MapleInventoryType.EQUIPPED.ordinal()].newList()) {
            ...
            if (item instanceof Equip) {
                ...
                if (MapleJob.is天使破壞者(getJob()) && ItemConstants.类型.套服(itemId)) {
                    itemId = 1051291; //ab def overall
                }
            }
            if (MapleJob.is天使破壞者(getJob())) {
                if (!ItemConstants.类型.套服(itemId) && !ItemConstants.类型.副手(itemId)
                        && !ItemConstants.类型.武器(itemId) && !ItemConstants.类型.勳章(itemId)) {
                    continue;
                }
            }
            eq.put((byte) item.getPosition(), itemId);
...
//        client.getSession().write(EffectPacket.updateDeathCount(deathCount));
    ...
    public void showPlayerStats(MapleCharacter player, boolean advanced) {
        ...
        builder.append(" 职业:").append(MapleJob.getById(player.getJob()).name());
