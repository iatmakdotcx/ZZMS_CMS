...
public class MapleClient implements Serializable {
    ...
    private ArrayList<Integer> charPos = new  ArrayList<Integer>();
...
//                System.out.println("密码哈希值 - " + passhash);
...
//            }
    ...
    public final void unLockDisconnect() {
        getSession().write(CWvsContext.broadcastMsg(1, "当前账号在别处登入\r\n若不是你本人操作请及时更改密码。"));
    ...
    public final void disconnect(final boolean removeInChannelServer, final boolean fromCS, final boolean shutdown) {
        if (player != null) {
            System.out.println(getSessionIPAddress() + "下线处理(已上角色)");
            ...
            System.out.println(getSessionIPAddress() + "下线处理(未上角色)");
        }
    ...
    public final void sendPing() {
        sendPing(180000); //默认3分钟 自动断线
    }

    public final void sendPing(int time) {
        ...
        PingTimer.getInstance().schedule(() -> {
            try {
                if (getLatency() < 0) {
                    System.out.print("系统自动断开未响应的客户端 : "+getSession().getLocalAddress());
