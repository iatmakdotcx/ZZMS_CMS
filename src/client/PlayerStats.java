...
public class PlayerStats implements Serializable {
    ...
    private void handlePassiveSkills(MapleCharacter chra) {
        ...
        } else if (MapleJob.is末日反抗軍(job) && !MapleJob.is惡魔(job) && !MapleJob.is尖兵(job)) {
        ...
        if (MapleJob.is英雄(job)) {
            ...
            bof = chra.getPassiveLevel(bx);
            if (bof > 0 && ItemConstants.类型.盾牌(chra.getInventory(MapleInventoryType.EQUIPPED).getItem((short) -10).getItemId())) {
                eff = bx.getEffect(bof);
...
//                    percent_boss_damage_rate += eff.getBossDamage();
    ...
    private void 输出未处理技能(int job) {
        List<Integer> skills = SkillFactory.getSkillsByJob(job);
        block3 : for (int i = 1; i < skills.size(); ++i) {
            Skill skill = SkillFactory.getSkill(skills.get(i));
            switch (skill.getId()) {
                case 93: 
                case 104: 
                case 110: 
                ...
                default: {
                    if (!skill.isBuffSkill()) continue block3;
                    System.out.println("未处理技能:" + skill.getId() + "  名: " + skill.getName() + "  类型: " + skill.getSkillType());
                    continue block3;
                }
            }
        }
        System.err.println("职业未处理技能:" + job);
    }
    ...
    private void CalcPassive_SharpEye(final MapleCharacter player) {
        ...
        } else if (MapleJob.is双弩精灵(player.getJob())) {
    ...
    public final void relocHeal(MapleCharacter chra) {
        ...
        } else if (MapleJob.is双弩精灵(playerjob)) {
    ...
    public final void connectData(final MaplePacketLittleEndianWriter mplew, final MapleCharacter chr) {
        ...
        mplew.writeInt(mp);
        mplew.writeInt(GameConstants.fixDemonForce(chr)); //maxmp
    }
    ...
    public final boolean isRangedJob(final int job) {
        return MapleJob.is蒼龍俠客(job) || MapleJob.is双弩精灵(job) || MapleJob.is重砲指揮官(job) || job == 400 || (job / 10 == 52) || (job / 100 == 3) || (job / 100 == 13) || (job / 100 == 14) || (job / 100 == 33) || (job / 100 == 35) || (job / 10 == 41);
    }
    ...
    public int getLimitBreak(MapleCharacter chra) {
        ...
        if (weapon != null) {
            ...
            if ((subweapon != null) && (ItemConstants.类型.武器(subweapon.getItemId()))) {
