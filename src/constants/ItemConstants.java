...
//            damageSkin.put(2433456, 21); //韓文的傷害字型
    ...
    public static class 卷軸 {
        ...
        public static boolean canScroll(final int itemId) {
            return (itemId / 100000 != 19 && itemId / 100000 != 16) || 类型.機器人心臟(itemId); //no mech/taming/dragon/心臟
        }
    ...
    public static class 方塊 {
        ...
        public static boolean canUseCube(Equip eq, int cubeId) {
            switch (cubeId) {
                case 2711007://10週年武器專用方塊
                    if (套裝.get10YearSet().contains(eq.getItemId()) && 类型.武器(eq.getItemId())) {
                        return true;
                    }
                    return false;
                ...
                case 5062102://[6週年]奇幻方塊
                    if (套裝.get6YearSet().contains(eq.getItemId()) && 类型.武器(eq.getItemId())) {
                        return true;
                    }
                    return false;
        ...
        public static int getCubeType(int itemId) {
            ...
            switch (itemId) {
                ...
                case 2711001://奇怪的方塊(英雄初心者,說明上是傳說,實際只能洗到稀有)
        ...
        public static boolean optionTypeFits(final int optionType, final int itemId) {
            switch (optionType) {
                ...
                case 20: // 除了配飾和武器的全部裝備
                    return !类型.飾品(itemId) && !类型.武器(itemId);
                case 40: // 配飾
                    return 类型.飾品(itemId);
                case 51: // 帽子
                    return 类型.帽子(itemId);
                case 52: // 披風
                    return 类型.披風(itemId);
                case 53: // 上衣、褲子與套服
                    return 类型.上衣(itemId) || 类型.套服(itemId) || 类型.褲裙(itemId);
                case 54: // 手套
                    return 类型.手套(itemId);
                case 55: // 鞋子
                    return 类型.鞋子(itemId);
    ...
    public static class 类型 {
        ...
        public static boolean 特效戒指(int itemid) {
            return 友谊戒指(itemid) || 戀人戒指(itemid) || 結婚戒指(itemid);
        }
