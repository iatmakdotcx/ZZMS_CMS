...
public class ChannelServer {
    ...
    private final int cashRate = 1, traitRate = 10, BossDropRate = 3;//乐豆掉宝倍率, 性向倍率, BOSS掉宝倍率(未使用)
...
//        acceptor = new NioSocketAcceptor(Runtime.getRuntime().availableProcessors() + 1);
        ...
        try {
            ...
            System.err.println("无法绑定" + port + "端口 (频道: " + channel + ")" + e);
        }
    }
    ...
    public final void shutdown() {
        ...
        broadcastPacket(CWvsContext.broadcastMsg(0, "当前频道将进行维护。"));
        // dc all clients by hand so we get sessionClosed...
        shutdown = true;

        System.out.println("频道 " + channel + " 正在储存角色数据");

        getPlayerStorage().disconnectAll();

        System.out.println("频道 " + channel + " 正在解除端口绑定");
    ...
    public final void setShutdown() {
        this.shutdown = true;
        System.out.println("频道 " + channel + " 已被设置为关闭并关闭雇佣商人...");
    }

    public final void setFinishShutdown() {
        this.finishedShutdown = true;
        System.out.println("频道 " + channel + " 关闭完成");
    }
