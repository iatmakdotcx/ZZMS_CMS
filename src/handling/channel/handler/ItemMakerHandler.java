...
public class ItemMakerHandler {
    ...
    private static int[] getCrystal(final int itemid, final int level) {
        ...
        if (ItemConstants.类型.武器(itemid) || ItemConstants.类型.雙刀(itemid) || ItemConstants.类型.套服(itemid)) {
    ...
    public static final void CraftComplete(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (craftID == 92049000) { //disassembling
            ...
            toGet = 4031016;
            quantity = (short) Randomizer.rand(3, ItemConstants.类型.武器(itemId) || ItemConstants.类型.雙刀(itemId) || ItemConstants.类型.套服(itemId) ? 11 : 7);
            if (reqLevel <= 60) {
                toGet = 4021013;
            } else if (reqLevel <= 90) {
                toGet = 4021014;
