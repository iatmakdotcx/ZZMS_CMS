...
public class MovementParse {
    ...
    public static List<LifeMovementFragment> parseMovement(final LittleEndianAccessor lea, final int kind, MapleCharacter chr) {
        ...
        for (byte i = 0; i < numCommands; i++) {
            ...
            switch (command) {
                ...
                default: {
                    ...
                    FileoutputUtil.log(FileoutputUtil.Movement_Log, moveMsg + "(" + chr.getName() + ") 职业：" + MapleJob.getName(MapleJob.getById(chr.getJob())) + "(" + chr.getJob() + ")  未知移動封包 剩餘次數: " + (numCommands - res.size()) + " 移動類型: 0x" + HexTool.toString(command) + ", 封包: " + lea.toString(true));
                    return null;
                }
            }
            if (kind == 7) {
                if (chr.isShowErr()) {
                    chr.showInfo("移動", true, "攻擊怪物" + "(" + chr.getName() + ") 职业：" + MapleJob.getName(MapleJob.getById(chr.getJob())) + "移動類型: 0x" + HexTool.toString(command));
                }
                FileoutputUtil.log(FileoutputUtil.Movement_Log, "攻擊怪物" + "(" + chr.getName() + ") 职业：" + MapleJob.getName(MapleJob.getById(chr.getJob())) + "移動類型: 0x" + HexTool.toString(command) + ", 封包: " + lea.toString(true));
            }
        }
        ...
        if (numCommands != res.size()) {
            ...
            FileoutputUtil.log(FileoutputUtil.Movement_Log, "循環次數[" + numCommands + "]和實際上獲取的循環次數[" + res.size() + "]不符 " + "(" + chr.getName() + ") 职业：" + MapleJob.getName(MapleJob.getById(chr.getJob())) + "(" + chr.getJob() + "移動封包 剩餘次數: " + (numCommands - res.size()) + "  封包: " + lea.toString(true));
