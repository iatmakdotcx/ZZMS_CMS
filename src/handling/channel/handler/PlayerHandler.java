...
//                    c.getSession().write(AngelicPacket.showRechargeEffect());
    ...
    public static void rangedAttack(LittleEndianAccessor slea, MapleClient c, final MapleCharacter chr) {
        ...
        boolean noBullet = (chr.getJob() >= 300 && chr.getJob() <= 322) || (chr.getJob() >= 3500 && chr.getJob() <= 3512) || MapleJob.is重砲指揮官(chr.getJob()) || MapleJob.is蒼龍俠客(chr.getJob()) || MapleJob.is幻影俠盜(chr.getJob()) || MapleJob.is双弩精灵(chr.getJob()) || MapleJob.is神之子(chr.getJob()) || MapleJob.is林芝林(chr.getJob());
        if (attack.skill != 0) {
