...
//            case SET_ITEMS: {
                ...
                if (chr.getTrade() != null && item != null) {
                    if ((quantity <= item.getQuantity() && quantity >= 0) || ItemConstants.类型.可充值道具(item.getItemId())) {
...
//                    }
                    if (ivItem.getQuantity() >= bundles_perbundle) {
                        ...
                        if (ItemConstants.类型.可充值道具(ivItem.getItemId())) {
                            // Ignore the bundles
...
//            case BUY_ITEM_HIREDMERCHANT: { // Buy and Merchant buy
            ...
            case BUY_ITEM_HIREDMERCHANT: { // Buy and Merchant buy
                ...
                if (tobuy.bundles < quantity || (tobuy.bundles % quantity != 0 && ItemConstants.类型.装备(tobuy.item.getItemId())) // Buying
