...
//    }
    public final void gainItem(final int id, final short quantity, final boolean randomStats, final long period, boolean hours, final int slots, final String owner, final MapleClient cg, final boolean show) {
        if (quantity >= 0) {
            ...
            if (type.equals(MapleInventoryType.EQUIP) && !ItemConstants.类型.可充值道具(id)) {
                ...
                item.setGMLog("从脚本 " + this.id + "(" + id2 + ")[" + script + "] 中获得, 时间 " + FileoutputUtil.CurrentReadable_Time());
                final String name = ii.getName(id);
                if (id / 10000 == 114 && name != null && name.length() > 0) { //medal
                    final String msg = "<" + name + ">获得称号。";
                ...
                MapleInventoryManipulator.addById(cg, id, quantity, owner == null ? "" : owner, pet, period, hours ? MapleInventoryManipulator.HOUR : MapleInventoryManipulator.DAY, "从脚本 " + this.id + "(" + id2 + ")[" + script + "] 中获得, 时间 " + FileoutputUtil.CurrentReadable_Time());
            }
        } else {
...
//        System.err.println(curtime);
    ...
    public void gainPet(int id, String name, int level, int closeness, int fullness, long period, short flags) {
        ...
        try {
            MapleInventoryManipulator.addById(c, id, (short) 1, "", MaplePet.createPet(id, name, level, closeness, fullness, MapleInventoryIdentifier.getInstance(), id == 5000054 ? (int) period : 0, flags, 0), 45, MapleInventoryManipulator.DAY, "从脚本 " + id + "(" + id2 + ")[" + script + "] 中获得, 时间:" + FileoutputUtil.CurrentReadable_Date());
...
//    }
    ...
    public void changeDamageSkinByItem(int itemId) {
        if (!ItemConstants.伤害字型.isDamageSkin(itemId)) {
            return;
        }
        changeDamageSkin(ItemConstants.伤害字型.getDamageSkinNumberByItem(itemId), true);
    }
    ...
    public void changeDamageSkin(int skinid, boolean show) {
        if (MapleJob.is神之子(c.getPlayer().getJob())) {
            c.getPlayer().dropMessage(-9, "神之子无法套用伤害字型。");
            return;
        }
        if (skinid == -1) {
            c.getPlayer().dropMessage(-9, "出现未知错误");
            return;
        }
        c.getPlayer().setDamageSkin(skinid);
        if (show) {
            c.getPlayer().dropMessage(-9, "伤害字型已更变。");
        }
