    ...
    public final void start(final MapleClient c, final int npc, String script) {
        ...
        try {
            if (c.getPlayer().isShowInfo()) {
                c.getPlayer().showInfo("NPC脚本", false, "开启对话，NPC：" + npc + " 特殊脚本：" + script + c.getPlayer().getMap());
            }
            if (!cms.containsKey(c) && c.canClickNPC()) {
                ...
                if (iv == null) {
                    if (c.getPlayer().isShowErr()) {
                        c.getPlayer().showInfo("NPC脚本", true, "找不到NPCID:" + npc + " 特殊脚本:" + script + c.getPlayer().getMap());
                    }
                    System.out.println("\r\n找不到NPCID:" + npc + " 特殊脚本:" + script + c.getPlayer().getMap() + "\r\n");
                ...
                if (c.getPlayer().isShowErr()) {
                    c.getPlayer().showInfo("NPC脚本", true, "无法执行脚本:已有脚本执行-" + cms.containsKey(c) + " | 允许执行脚本-" + c.canClickNPC());
                }
                ...
                if (cm == null || (cm.getType() != ScriptType.ON_USER_ENTER && cm.getType() != ScriptType.ON_FIRST_USER_ENTER)) {
                    c.getPlayer().dropMessage(-1, "你当前已经和1个NPC对话了. 如果不是请输入 @解卡 指令进行解卡。");
                }
            }
        } catch (final ScriptException | NoSuchMethodException e) {
            System.err.println("执行NPC脚本出错 : NPC - " + npc + " 特殊脚本 - " + script + " ." + e);
            FileoutputUtil.log(FileoutputUtil.ScriptEx_Log, "\r\n\r\n执行NPC脚本出错, NPC - " + npc + " 特殊脚本 - " + script + " ." + e + "\r\n\r\n");
            dispose(c);
...
//                    System.err.println("sel " + selection);
                ...
                switch (cm.getType()) {
                    case NPC:
                        str = "执行NPC脚本出错 : NPC - " + cm.getNpc() + " 特殊脚本 - " + cm.getScript();
                        break;
                    case ITEM:
                        str = "执行道具脚本出错 : NPC - " + cm.getNpc() + " 脚本 - " + cm.getScript();
                        break;
                    case ON_USER_ENTER:
                        str = "执行地图onUserEnter脚本出错 : 脚本 - " + cm.getScript();
                        break;
                    case ON_FIRST_USER_ENTER:
                        str = "执行地图onFirstUserEnter脚本出错 : 脚本 - " + cm.getScript();
                        break;
                    default:
                        str = "执行脚本出错 : NPC - " + cm.getNpc() + " 脚本 - " + cm.getScript();
                }
    ...
    public final void startQuest(final MapleClient c, final int npc, final int quest) {
        if (!MapleQuest.getInstance(quest).canStart(c.getPlayer(), null)) {
            if (c.getPlayer().isShowErr()) {
                c.getPlayer().showInfo("任务开始脚本", true, "无法开始任务 NPC：" + npc + " 任务：" + MapleQuest.getInstance(quest));
            }
            return;
        }
        final Lock lock = c.getNPCLock();
        ...
        try {
            if (c.getPlayer().isShowInfo()) {
                c.getPlayer().showInfo("任务开始脚本", false, "脚本 - 开始任务 NPC：" + npc + " 任务：" + MapleQuest.getInstance(quest));
            }
            if (!cms.containsKey(c) && c.canClickNPC()) {
                final Invocable iv = getInvocable("quest/" + quest + ".js", c, true);
                if (iv == null) {
                    if (c.getPlayer().isShowErr()) {
                        c.getPlayer().showInfo("任务开始脚本", true, "找不到脚本任务 NPC：" + npc + " 任务：" + MapleQuest.getInstance(quest) + c.getPlayer().getMap());
                    }
                    dispose(c);
                    return;
                }
                ...
                c.getPlayer().showInfo("任务开始脚本", true, "无法执行脚本:已有脚本执行-" + cms.containsKey(c) + " | 允许执行脚本-" + c.canClickNPC());
            }
        } catch (final ScriptException | NoSuchMethodException e) {
            System.err.println("执行任务开始脚本出错 : NPC - " + npc + "任务 - " + MapleQuest.getInstance(quest) + " ." + e);
            FileoutputUtil.log(FileoutputUtil.ScriptEx_Log, "\r\n\r\n执行任务开始脚本出错 : NPC - " + npc + "任务 - " + MapleQuest.getInstance(quest) + " ." + e + "\r\n\r\n");
            dispose(c);
    ...
    public final void startQuest(final MapleClient c, final byte mode, final byte type, final int selection) {
        ...
        try {
            ...
            System.err.println("执行任务开始脚本出错 : NPC - " + cm.getNpc() + "任务 - " + MapleQuest.getInstance(cm.getQuest()) + " ." + e);
            FileoutputUtil.log(FileoutputUtil.ScriptEx_Log, "\r\n\r\n执行任务开始脚本出错 : NPC - " + cm.getNpc() + "任务 - " + MapleQuest.getInstance(cm.getQuest()) + " ." + e + "\r\n\r\n");
            dispose(c);
    ...
    public final void endQuest(final MapleClient c, final int npc, final int quest, final boolean customEnd) {
        if (!customEnd && !MapleQuest.getInstance(quest).canComplete(c.getPlayer(), null)) {
            if (c.getPlayer().isShowErr()) {
                c.getPlayer().showInfo("任务完成脚本", true, "无法完成任务 NPC：" + npc + " 任务：" + MapleQuest.getInstance(quest));
            }
            return;
        }
        final Lock lock = c.getNPCLock();
        ...
        try {
            if (c.getPlayer().isShowInfo()) {
                c.getPlayer().showInfo("任务完成脚本", false, "脚本 - 完成任务 NPC：" + npc + " 任务：" + MapleQuest.getInstance(quest));
            }
            if (!cms.containsKey(c) && c.canClickNPC()) {
                final Invocable iv = getInvocable("quest/" + quest + ".js", c, true);
                if (iv == null) {
                    if (c.getPlayer().isShowErr()) {
                        c.getPlayer().showInfo("任务完成脚本", true, "找不到脚本任务 NPC：" + npc + " 任务：" + MapleQuest.getInstance(quest) + c.getPlayer().getMap());
                    }
                    dispose(c);
                    return;
                }
            ...
            System.err.println("执行任务完成脚本出错 : NPC - " + npc + "任务 - " + MapleQuest.getInstance(quest) + " ." + e);
            FileoutputUtil.log(FileoutputUtil.ScriptEx_Log, "\r\n\r\n执行任务完成脚本出错 : NPC - " + npc + "任务 - " + MapleQuest.getInstance(quest) + " ." + e + "\r\n\r\n");
            dispose(c);
    ...
    public final void endQuest(final MapleClient c, final byte mode, final byte type, final int selection) {
        ...
        try {
            ...
            System.err.println("执行任务完成脚本出错 : NPC - " + cm.getNpc() + "任务 - " + MapleQuest.getInstance(cm.getQuest()) + " ." + e);
            FileoutputUtil.log(FileoutputUtil.ScriptEx_Log, "\r\n\r\n执行任务完成脚本出错 : NPC - " + cm.getNpc() + "任务 - " + MapleQuest.getInstance(cm.getQuest()) + " ." + e + "\r\n\r\n");
            dispose(c);
            notice(c, cm.getNpc(), String.valueOf(cm.getQuest()), cm.getType());
        } finally {
            lock.unlock();
    ...
    public final void startItemScript(final MapleClient c, final int npc, final String script) {
        ...
        try {
            if (c.getPlayer().isShowInfo()) {
                c.getPlayer().showInfo("道具脚本", false, "开启对话，NPC：" + npc + " 道具脚本：" + script);
            }
            if (!cms.containsKey(c) && c.canClickNPC()) {
                final Invocable iv = getInvocable("item/" + script + ".js", c, true);
                if (iv == null) {
                    if (c.getPlayer().isShowErr()) {
                        c.getPlayer().dropMessage(5, "找不到道具脚本:" + script);
                        c.getPlayer().showInfo("道具脚本", true, "找不到道具脚本:" + script);
                    }
                    System.out.println("\r\n找不到道具脚本:" + script + "\r\n");
                    dispose(c);
                    return;
                ...
                c.getPlayer().showInfo("道具脚本", true, "无法执行脚本:已有脚本执行-" + cms.containsKey(c) + " | 允许执行脚本-" + c.canClickNPC());
            }
        } catch (final ScriptException | NoSuchMethodException e) {
            System.err.println("执行道具脚本出错 : NPC - " + npc + " 脚本 - " + script + " ." + e);
            FileoutputUtil.log(FileoutputUtil.ScriptEx_Log, "\r\n\r\n执行道具脚本出错, NPC - " + npc + " 脚本 - " + script + " ." + e + "\r\n\r\n");
            dispose(c);
    ...
    public final void onUserEnter(final MapleClient c, final String script) {
        ...
        try {
            if (c.getPlayer().isShowInfo()) {
                c.getPlayer().showInfo("onUserEnter脚本", false, "开始onUserEnter脚本：" + script + c.getPlayer().getMap());
            }
            if (!cms.containsKey(c)) {
                final Invocable iv = getInvocable("map/onUserEnter/" + script + ".js", c, true);
                if (iv == null) {
                    if (c.getPlayer().isShowErr()) {
                        c.getPlayer().showInfo("onUserEnter脚本", true, "找不到onUserEnter脚本:" + script + c.getPlayer().getMap());
                    }
                    System.out.println("\r\n找不到onUserEnter脚本:" + script + c.getPlayer().getMap() + "\r\n");
                    dispose(c);
                    return;
                }
                final ScriptEngine scriptengine = (ScriptEngine) iv;
                ...
                c.getPlayer().showInfo("onUserEnter脚本", true, "无法执行脚本:已有脚本执行-" + cms.containsKey(c) + "(" + cms.get(c).getScript() + ")");
            }
        } catch (final ScriptException | NoSuchMethodException e) {
            System.err.println("执行onUserEnter脚本出错 : " + script + ". " + e);
            FileoutputUtil.log(FileoutputUtil.ScriptEx_Log, "执行onUserEnter脚本出错 : " + script + ". " + e);
            dispose(c);
    ...
    public final void onFirstUserEnter(final MapleClient c, final String script) {
        ...
        try {
            if (c.getPlayer().isShowInfo()) {
                c.getPlayer().showInfo("onFirstUserEnter脚本", false, "开始onFirstUserEnter脚本：" + script + c.getPlayer().getMap());
            }
            if (!cms.containsKey(c)) {
                final Invocable iv = getInvocable("map/onFirstUserEnter/" + script + ".js", c, true);
                if (iv == null) {
                    if (c.getPlayer().isShowErr()) {
                        c.getPlayer().showInfo("onFirstUserEnter脚本", true, "找不到onFirstUserEnter脚本:" + script + c.getPlayer().getMap());
                    }
                    System.out.println("\r\n找不到onFirstUserEnter脚本:" + script + c.getPlayer().getMap() + "\r\n");
                    dispose(c);
                    return;
                }
                final ScriptEngine scriptengine = (ScriptEngine) iv;
                ...
                c.getPlayer().showInfo("onFirstUserEnter脚本", true, "无法执行脚本:已有脚本执行-" + cms.containsKey(c));
            }
        } catch (final ScriptException | NoSuchMethodException e) {
            System.err.println("执行地图onFirstUserEnter脚本出错 : " + script + ". " + e);
            FileoutputUtil.log(FileoutputUtil.ScriptEx_Log, "执行地图onFirstUserEnter脚本出错 : " + script + ". " + e);
            dispose(c);
        ...
        if (npccm != null) {
            ...
            switch (npccm.getType()) {
                case NPC:
                    ...
                    c.removeScriptEngine("scripts/特殊/notcoded.js");
                    break;
                case ITEM:
    ...
    private void notice(MapleClient c, int npcId, String script, ScriptType type) {
        ...
        switch (type) {
            case NPC:
                str = "NPC脚本出错-NPC:" + npcId + (script != null ? " 特殊脚本:" + script : "");
                break;
            case ITEM:
                str = "道具脚本出错-NPC:" + npcId + " 脚本:" + script;
                break;
            case ON_USER_ENTER:
                str = "onUserEnter脚本出错-脚本:" + script;
                break;
            case ON_FIRST_USER_ENTER:
                str = "onFirstUserEnter脚本出错-脚本:" + script;
                break;
            default:
                str = "脚本出错-NPC:" + npcId + " 脚本:" + script;
        }
