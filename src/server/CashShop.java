...
public class CashShop implements Serializable {
    ...
    public void checkExpire(MapleClient c) {
        ...
        for (Item item : inventory) {
            if (item != null && !ItemConstants.类型.宠物(item.getItemId()) && item.getExpiration() > 0 && item.getExpiration() < System.currentTimeMillis()) {
                toberemove.add(item);
            }
        }
    ...
    public Item toItem(CashItemInfo cItem, int uniqueid, String gift) {
        ...
        if (ItemConstants.类型.宠物(cItem.getId())) {
            period = 90;
        }
        ...
        if (GameConstants.getInventoryType(cItem.getId()) == MapleInventoryType.EQUIP) {
            ...
            eq.setGiftFrom(gift);
            if (ItemConstants.类型.特效戒指(cItem.getId()) && uniqueid > 0) {
    ...
    public Item toItem(CashItem cItem, int uniqueid, String gift) {
        ...
        if (ItemConstants.类型.宠物(cItem.getItemId())) {
            period = 90;
        }
        ...
        Item ret;
        if (GameConstants.getInventoryType(cItem.getItemId()) == MapleInventoryType.EQUIP && !ItemConstants.类型.宠物(cItem.getItemId())) {
            ...
            eq.setGMLog("商場購買 - SN:" + cItem.getSN() + " 時間: " + FileoutputUtil.CurrentReadable_Date());
            eq.setGiftFrom(gift);
            if (ItemConstants.类型.特效戒指(cItem.getItemId()) && uniqueid > 0) {
                MapleRing ring = MapleRing.loadFromDb(uniqueid);
                if (ring != null) {
                    eq.setRing(ring);
                }
