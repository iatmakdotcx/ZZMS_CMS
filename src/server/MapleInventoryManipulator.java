...
public class MapleInventoryManipulator {
    ...
    public static void addRing(MapleCharacter chr, int itemId, int ringId, int sn, String partner) {
        ...
        chr.getClient().getSession().write(CSPacket.sendBoughtRings(ItemConstants.类型.戀人戒指(itemId), ring, sn, chr.getClient().getAccID(), partner));
    }
    ...
    public static byte addId(MapleClient c, int itemId, short quantity, String owner, MaplePet pet, long period, int periodMod, String gmLog) {
        ...
        if (ItemConstants.类型.宠物(itemId) && pet == null) {
            ...
            if (!ItemConstants.类型.可充值道具(itemId)) {
                ...
                while (quantity > 0) {
                    ...
                    if (newQ != 0) {
                        ...
                        if (ItemConstants.类型.可充值道具(itemId) && quantity == 0) {
                            break;
                        }
                    } else {
    ...
    public static boolean checkSpace(final MapleClient c, final int itemid, int quantity, final String owner) {
        ...
        if (quantity <= 0 && !ItemConstants.类型.可充值道具(itemid)) {
            return false;
        }
        final MapleInventoryType type = GameConstants.getInventoryType(itemid);
        ...
        if (!type.equals(MapleInventoryType.EQUIP)) {
            ...
            if (slotMax > 0 && !ItemConstants.类型.可充值道具(itemid)) {
    ...
    public static boolean removeFromSlot(final MapleClient c, final MapleInventoryType type, final short slot, short quantity, final boolean fromDrop, final boolean consume) {
        ...
        if (item != null) {
            final boolean allowZero = consume && ItemConstants.类型.可充值道具(item.getItemId());
    ...
    public static void move(final MapleClient c, final MapleInventoryType type, final short src, final short dst) {
        ...
        if (!type.equals(MapleInventoryType.EQUIP) && initialTarget != null
                ...
                && !ItemConstants.类型.可充值道具(source.getItemId())
    ...
    public static void equip(final MapleClient c, final short src, short dst) {
        ...
        if (ItemConstants.类型.管理員裝備(source.getItemId()) && !c.getPlayer().isStaff()) {
        ...
        if (dst > -1200 && dst < -999 && !ItemConstants.类型.龍裝備(source.getItemId()) && !ItemConstants.类型.機械(source.getItemId())) {
            if (chr.isShowErr()) {
        ...
        if (ItemConstants.类型.副手(source.getItemId()) && dst != -10) {
            c.getSession().write(CWvsContext.enableActions());
            return;
        }
        if (ItemConstants.类型.武器(source.getItemId()) && dst != -11) {
            c.getSession().write(CWvsContext.enableActions());
            return;
        }
        ...
        if ((dst == -35) && (!ItemConstants.类型.能源(source.getItemId()))) {
        ...
        if (ItemConstants.类型.雙刀(source.getItemId()) || source.getItemId() / 10000 == 135) {
            dst = (byte) -10; //shield slot
        }
        if (ItemConstants.类型.龍裝備(source.getItemId()) && (chr.getJob() < 2200 || chr.getJob() > 2218)) {
            c.getSession().write(CWvsContext.enableActions());
            return;
        }

        if (ItemConstants.类型.機械(source.getItemId()) && (chr.getJob() < 3500 || chr.getJob() > 3512)) {
            c.getSession().write(CWvsContext.enableActions());
            return;
        }
        ...
        switch (dst) {
            case -6: { // Bottom
                ...
                if (top != null && ItemConstants.类型.套服(top.getItemId())) {
            ...
            case -5: {
                ...
                if (bottom != null && ItemConstants.类型.套服(source.getItemId())) {
            ...
            case -10: { // Shield
                ...
                if (ItemConstants.类型.雙刀(source.getItemId())) {
                    if ((chr.getJob() != 900 && !MapleJob.is影武者(chr.getJob())) || weapon == null || !ItemConstants.类型.短劍(weapon.getItemId())) {
                        c.getSession().write(InventoryPacket.getInventoryFull());
                        c.getSession().write(InventoryPacket.getShowInventoryFull());
                        return;
                    }
                } else if (weapon != null && ItemConstants.类型.雙手武器(weapon.getItemId()) && !ItemConstants.类型.特殊副手(source.getItemId())) {
                    if (chr.getInventory(MapleInventoryType.EQUIP).isFull(-1)) {
                        c.getSession().write(InventoryPacket.getInventoryFull());
                        c.getSession().write(InventoryPacket.getShowInventoryFull());
                        return;
            ...
            case -11: { // Weapon
                Item weapon = chr.getInventory(MapleInventoryType.EQUIPPED).getItem((byte) -11);
                Item shield = chr.getInventory(MapleInventoryType.EQUIPPED).getItem((byte) -10);
                if (shield != null && ItemConstants.类型.雙手武器(source.getItemId()) && !ItemConstants.类型.特殊副手(shield.getItemId())) {
                    if (chr.getInventory(MapleInventoryType.EQUIP).isFull(-1)) {
                        c.getSession().write(InventoryPacket.getInventoryFull());
                        c.getSession().write(InventoryPacket.getShowInventoryFull());
                        return;
        ...
        if (ItemConstants.类型.武器(source.getItemId())) {
    ...
    public static boolean drop(final MapleClient c, MapleInventoryType type, final short src, short quantity, final boolean npcInduced) {
        ...
        final Item source = c.getPlayer().getInventory(type).getItem(src);
        if (quantity < 0 || source == null || src == -55 || (!npcInduced && ItemConstants.类型.宠物(source.getItemId())) || (quantity == 0 && !ItemConstants.类型.可充值道具(source.getItemId())) || c.getPlayer().inPVP()) {
            c.getSession().write(CWvsContext.enableActions());
            return false;
        }

        final short flag = source.getFlag();
        if (quantity > source.getQuantity() && !ItemConstants.类型.可充值道具(source.getItemId())) {
            c.getSession().write(CWvsContext.enableActions());
            return false;
        }
        ...
        if (quantity < source.getQuantity() && !ItemConstants.类型.可充值道具(source.getItemId())) {
            ...
            if (ItemConstants.类型.宠物(source.getItemId()) || ItemFlag.UNTRADABLE.check(flag) || ii.isDropRestricted(target.getItemId()) || ii.isAccountShared(target.getItemId())) {
            ...
            if (ItemConstants.类型.宠物(source.getItemId()) || ItemFlag.UNTRADABLE.check(flag) || ii.isDropRestricted(source.getItemId()) || ii.isAccountShared(source.getItemId())) {
