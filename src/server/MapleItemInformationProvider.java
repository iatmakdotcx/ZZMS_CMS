...
public class MapleItemInformationProvider {
    ...
    public final Item scrollEquipWithId(final Item equip, final Item scroll, final boolean ws, final MapleCharacter chr, final int vegas) {
        if (equip.getType() == 1) { // See Item.java
            ...
            if (ItemFlag.LUCKY_DAY.check(equip.getFlag()) && !ItemConstants.类型.潛能卷軸(scroll.getItemId()) && !ItemConstants.类型.附加潛能卷軸(scroll.getItemId()) && !ItemConstants.类型.裝備強化卷軸(scroll.getItemId()) && (!ItemConstants.类型.特殊卷軸(scroll.getItemId())) && !ItemConstants.类型.回真卷軸(scroll.getItemId())) {
                equip.setFlag((short) (equip.getFlag() - ItemFlag.LUCKY_DAY.getValue()));
            }
            final int added = lucksKey + craft;

            if (ItemConstants.类型.潛能卷軸(scroll.getItemId())) {
                success = 0;
                curse = 100;
                ...
                scrollType += "潛能附加捲軸";
            } else if (ItemConstants.类型.附加潛能卷軸(scroll.getItemId())) {
            ...
            } else if (ItemConstants.类型.裝備強化卷軸(scroll.getItemId())) {
                success = stats == null || !stats.containsKey("success") ? 0 : stats.get("success");
            ...
            } else if (ItemConstants.类型.回真卷軸(scroll.getItemId())) {
                success = stats == null || !stats.containsKey("success") ? 0 : stats.get("success");
                curse = noCursed || stats == null || !stats.containsKey("cursed") ? 0 : stats.get("cursed");
                scrollType = "回真捲軸";
            } else {
                if (ItemConstants.类型.幸運日卷軸(scroll.getItemId())) {
                    if (ItemConstants.类型.保護卷軸(scroll.getItemId())) {
                        scrollType = "幸運日+防爆捲軸";
                    } else {
                        scrollType = "幸運日捲軸";
                    }
                } else if (ItemConstants.类型.保護卷軸(scroll.getItemId())) {
                    scrollType = "防爆捲軸";
                } else if (ItemConstants.类型.安全卷軸(scroll.getItemId())) {
                    scrollType = "安全捲軸";
                } else if (ItemConstants.类型.卷軸保護卡(scroll.getItemId())) {
                    scrollType = "捲軸保護卡";
                } else if (ItemConstants.类型.白醫卷軸(scroll.getItemId())) {
                    scrollType = "白醫捲軸";
                } else if (ItemConstants.类型.混沌卷軸(scroll.getItemId())) {
                    scrollType = "混沌捲軸";
                } else {
                    scrollType = "普通捲軸";
                }
                success = stats == null || !stats.containsKey("success") ? ItemConstants.类型.提升卷(scroll.getItemId()) ? ItemConstants.卷軸.getSuccessTablet(scroll.getItemId(), nEquip.getLevel()) : 0 : stats.get("success");
                curse = stats == null || !stats.containsKey("cursed") ? ItemConstants.类型.提升卷(scroll.getItemId()) ? ItemConstants.卷軸.getCurseTablet(scroll.getItemId(), nEquip.getLevel()) : 0 : stats.get("cursed");
            ...
            if (ItemConstants.类型.潛能卷軸(scroll.getItemId()) || ItemConstants.类型.附加潛能卷軸(scroll.getItemId()) || ItemConstants.类型.裝備強化卷軸(scroll.getItemId()) || ItemConstants.类型.特殊卷軸(scroll.getItemId()) || ItemConstants.类型.回真卷軸(scroll.getItemId()) || Randomizer.nextInt(100) <= success) {
                if (ItemConstants.类型.幸運日卷軸(scroll.getItemId()) || ItemConstants.类型.保護卷軸(scroll.getItemId())) {
                    if (ItemConstants.类型.幸運日卷軸(scroll.getItemId())) {//幸運日
                ...
                } else if (ItemConstants.类型.安全卷軸(scroll.getItemId())) {//安全捲軸
                    short flag = nEquip.getFlag();
                    flag = (short) (flag | ItemFlag.SLOTS_PROTECT.getValue());
                    nEquip.setFlag(flag);
                } else if (ItemConstants.类型.卷軸保護卡(scroll.getItemId())) {//捲軸保護卡
                    short flag = nEquip.getFlag();
                ...
                } else if (ItemConstants.类型.回真卷軸(scroll.getItemId())) {//回真捲軸
                    if (equip.getType() != 1) {
                        return equip;
                    }
                    if (Randomizer.nextInt(100) > success) {
                    ...
                    nEquip.setFlag(flag);
                } else if (ItemConstants.类型.白醫卷軸(scroll.getItemId())) {//白醫捲軸
                    ...
                    int increase = Randomizer.nextBoolean() ? 1 : (ItemConstants.类型.樂觀混沌卷軸(scroll.getItemId())) || (isNegativeScroll(scroll.getItemId())) ? 1 : -1;
                ...
                if (!ItemConstants.类型.白醫卷軸(scroll.getItemId()) && !ItemConstants.类型.特殊卷軸(scroll.getItemId()) && !ItemConstants.类型.裝備強化卷軸(scroll.getItemId()) && !ItemConstants.类型.潛能卷軸(scroll.getItemId()) && !ItemConstants.类型.附加潛能卷軸(scroll.getItemId()) && 2040727 != scroll.getItemId() && 2041058 != scroll.getItemId()) {
                    if (ItemConstants.类型.阿斯旺卷軸(scroll.getItemId())) {
                ...
                if (!ws && !ItemConstants.类型.白醫卷軸(scroll.getItemId()) && 2040727 != scroll.getItemId() && 2041058 != scroll.getItemId()) {
                    short oldFlag = nEquip.getFlag();
                    if (ItemFlag.SLOTS_PROTECT.check(oldFlag)) {
                        nEquip.setFlag((short) (oldFlag - ItemFlag.SLOTS_PROTECT.getValue()));
                        chr.showMessage(11, "由卷軸的效果來，不會扣掉可以升級的次數。");
                    } else if (ItemConstants.类型.阿斯旺卷軸(scroll.getItemId())) {
                        nEquip.setUpgradeSlots((byte) (nEquip.getUpgradeSlots() - eqstats.get("tuc")));
                    } else {
                        nEquip.setUpgradeSlots((byte) (nEquip.getUpgradeSlots() - 1));
                    }
    ...
    public void finalizeEquipData(ItemInformation item) {
        ...
        if (item.equipStats.size() > 0) {
            ...
            if (item.equipStats.get("cash") != null && item.eq.getCharmEXP() <= 0) { //set the exp
                ...
                if (ItemConstants.类型.武器(itemId) || identifier == 106) { //weapon overall
                    exp = 60;
                } else if (identifier == 100) { //hats
                    exp = 50;
                } else if (ItemConstants.类型.飾品(itemId) || identifier == 102 || identifier == 108 || identifier == 107) { //gloves shoes accessory
                    exp = 40;
