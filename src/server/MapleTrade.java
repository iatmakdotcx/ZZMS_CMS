...
public class MapleTrade {
    ...
    public final boolean setItems(MapleClient c, Item item, byte targetSlot, int quantity) {
        ...
        if ((this.partner == null) || (target == -1) || (ItemConstants.类型.宠物(item.getItemId())) || (isLocked()) || ((GameConstants.getInventoryType(item.getItemId()) == MapleInventoryType.EQUIP) && (quantity != 1))) {
            return false;
        }
        short flag = item.getFlag();
