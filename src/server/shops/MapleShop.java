...
public class MapleShop {
    ...
    public void buy(MapleClient c, short slot, int itemId, short quantity) {
        ...
        if (index >= 0) {
            ...
            int price = (int) Math.max(Math.ceil(ii.getPrice(itemId) * (ItemConstants.类型.可充值道具(itemId) ? 1 : i.getQuantity())), 0.0D);
        ...
        if (item.getPrice() > 0 && item.getReqItem() == 0) {
            ...
            int price = ItemConstants.类型.可充值道具(itemId) || shopItemQ > 1 ? item.getPrice() : item.getPrice() * quantity;
            if (price >= 0 && c.getPlayer().getMeso() >= price) {
    ...
    public void sell(MapleClient c, MapleInventoryType type, byte slot, short quantity) {
        ...
        MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
        if ((ii.cantSell(item.getItemId())) || (ItemConstants.类型.宠物(item.getItemId()))) {
            return;
        }
    ...
    public void recharge(MapleClient c, byte slot) {
        ...
        if ((item == null) || (!ItemConstants.类型.可充值道具(item.getItemId()))) {
            return;
        }
        MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
        ...
        try {
            ...
            while (rs.next()) {
                if (ii.itemExists(rs.getInt("itemid"))) {
                    if (ItemConstants.类型.可充值道具(rs.getInt("itemid"))) {
